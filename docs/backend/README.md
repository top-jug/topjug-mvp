...
| Operations setting event | `SETTING_EVENT_NOT_FOUND`, `SETTING_EVENT_SECTOR_GYM_MISMATCH`, `OPS_GYM_SETTING_SECTOR_NOT_FOUND`, `OPS_GYM_SETTING_SECTOR_EXISTS`, `INVALID_SETTING_EVENT_TIME_RANGE`, `INVALID_SETTING_EVENT_TRANSITION`, `OPS_RESOURCE_CHANGED` |
...
- Operations setting-event mutations require the event `updatedAt`, enforce active sector/gym ownership, and retain soft-deleted rows while excluding them from operations and public reads. The operations console provides wall-wide setting-sector management plus the responsive month calendar, list, and editor. Referenced sectors are deactivated instead of deleted so historical schedules and records keep their labels; notification delivery remains a follow-up task.
