...
export default function BottomTabBar({ activeTab, onNavigate }: BottomTabBarProps) {
  const activeColor = 'rgb(29 78 216)';
  const inactiveStroke = 'rgb(82 82 82)';

  return (
    <nav aria-label="주요 메뉴" className="mobile-bottom-nav fixed bottom-0 left-0 right-0 z-50 px-3 pt-1 pointer-events-auto">
      <div className="w-full h-full flex items-center justify-between">
        <div className="bg-white border border-neutral-200 rounded-full h-[68px] min-w-0 flex items-center justify-around px-2 flex-1 mr-2 shadow-[0_-4px_16px_rgba(15,23,42,0.08)]">
          <button onClick={() => onNavigate('home')} aria-current={activeTab === 'home' ? 'page' : undefined} className="flex min-w-0 flex-1 flex-col items-center gap-1.5 px-1">
            ...
            <span className={`whitespace-nowrap text-[12px] ${activeTab === 'home' ? 'text-blue-700' : 'text-neutral-600'}`}>홈</span>
          </button>
          <button onClick={() => onNavigate('gymSearch')} aria-current={activeTab === 'gymSearch' ? 'page' : undefined} className="flex min-w-0 flex-1 flex-col items-center gap-1.5 px-1">
            ...
            <span className={`whitespace-nowrap text-[12px] ${activeTab === 'gymSearch' ? 'text-blue-700' : 'text-neutral-600'}`}>암장</span>
          </button>
          <button onClick={() => onNavigate('calendar')} aria-current={activeTab === 'calendar' ? 'page' : undefined} className="flex min-w-0 flex-1 flex-col items-center gap-1.5 px-1">
            ...
            <span className={`whitespace-nowrap text-[12px] ${activeTab === 'calendar' ? 'text-blue-700' : 'text-neutral-600'}`}>캘린더</span>
          </button>
        </div>
