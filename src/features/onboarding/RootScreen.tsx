    ...
    return (
      <main className="mobile-screen flex items-center justify-center bg-[#f3faf8] px-5">
        <section className="w-full max-w-sm rounded-[28px] border border-teal-100 bg-white p-7 text-center shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
          ...
          <button type="button" onClick={() => void retry()} className="mt-6 h-12 w-full rounded-2xl bg-teal-700 text-sm font-bold text-white transition hover:bg-teal-800">
            다시 시도
          </button>
        </section>
      </main>
    ...
    <main className="mobile-screen overflow-hidden bg-[#f3faf8] text-neutral-950">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 pb-8 pt-[max(1.25rem,var(--mobile-safe-top))] sm:px-8 lg:px-12">
        <header className="flex items-center justify-between">
          <BrandLockup compact />
          <Link to={ONBOARDING_ROUTES.login} className="flex min-h-11 items-center rounded-full border border-teal-200 bg-white/80 px-5 text-sm font-bold text-teal-800 backdrop-blur transition hover:bg-white">
            로그인
          </Link>
        </header>
        ...
        <div className="grid flex-1 items-center gap-10 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-20 lg:py-16">
          <section>
            ...
            <h1 className="mt-6 max-w-[680px] text-[40px] font-black leading-[1.08] tracking-[-0.055em] text-neutral-950 sm:text-6xl lg:text-[68px]">
              오를수록 쌓이는<br /><span className="text-teal-700">나만의 클라이밍.</span>
            </h1>
            ...
            <div className="mt-8 grid gap-3 sm:flex">
              <Link to={ONBOARDING_ROUTES.register} className="flex h-14 items-center justify-center gap-2 rounded-2xl bg-teal-700 px-7 text-base font-bold text-white shadow-[0_14px_30px_rgba(15,118,110,0.22)] transition hover:bg-teal-800">
                무료로 시작하기 <ChevronRight size={19} />
              </Link>
              <Link to={ONBOARDING_ROUTES.gyms} className="flex h-14 items-center justify-center gap-2 rounded-2xl border border-neutral-200 bg-white px-7 text-base font-bold text-neutral-800 transition hover:border-teal-200 hover:text-teal-700">
          ...
          <section className="relative mx-auto w-full max-w-[470px] lg:mx-0" aria-label="TopJug 주요 기능">
            ...
            <div className="relative rounded-[36px] border border-white/80 bg-white/85 p-5 shadow-[0_30px_80px_rgba(15,118,110,0.14)] backdrop-blur sm:p-7">
              <div className="flex items-center gap-4 border-b border-neutral-100 pb-5">
                ...
                <div>
                  <div className="text-xs font-bold tracking-[0.14em] text-teal-700">YOUR NEXT SEND</div>
                  <h2 className="mt-1 text-xl font-black tracking-[-0.035em]">오늘은 어디를 오를까요?</h2>
                </div>
              </div>
              <div className="mt-5 grid gap-3">
                <FeatureLink to={ONBOARDING_ROUTES.gyms} icon={<MapPinned size={22} />} title="암장 탐색" description="가까운 암장과 상세 정보를 찾아보세요" />
                <FeatureLink to={ONBOARDING_ROUTES.calendar} icon={<CalendarDays size={22} />} title="세팅 캘린더" description="새로운 벽과 세팅 일정을 미리 확인하세요" />
        ...
        <footer className="text-center text-xs text-neutral-600 sm:text-left">암장 탐색과 세팅 일정은 로그인 없이 이용할 수 있어요.</footer>
      </div>
    </main>
  );
}
