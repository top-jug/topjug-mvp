...
import { ArrowLeft, Bell, CheckCircle2, Clock3, RefreshCw, Save } from 'lucide-react';
...
export function OperationsGymEditor() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><Link to="/ops/gyms" className="inline-flex items-center gap-1 text-sm font-bold text-slate-500 hover:text-blue-600"><ArrowLeft className="h-4 w-4" />암장 목록</Link><h2 className="mt-3 text-2xl font-black">{editing ? '암장 정보 편집' : '새 암장 등록'}</h2>{gym && <p className="mt-1 text-sm text-slate-500">ID {gym.id}</p>}</div>{gym && <div className="flex flex-col gap-2 sm:items-end"><div className="flex flex-col gap-2 sm:flex-row"><Link to={`/gyms/${gym.id}`} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700">사용자 화면 보기</Link><Link to={`/ops/gyms/${gym.id}/hours`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-black text-white"><Clock3 className="h-4 w-4" />운영시간 관리</Link></div><div className="rounded-xl bg-slate-100 px-4 py-3 text-sm"><span className="font-bold text-slate-500">마지막 확인 </span><span className="font-black">{dateTime(gym.lastVerifiedAt)}</span><span className="mx-2 text-slate-300">·</span><span className="font-bold text-slate-500">수정 </span><span className="font-black">{dateTime(gym.updatedAt)}</span></div></div>}</div>
      ...
      {editing && gym && <section className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-[1fr_auto_auto] md:items-end"><Field label="운영 상태"><select value={status} onChange={(event) => setStatus(event.target.value as GymOperationStatus)} className={inputClass}>{Object.entries(operationStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><button disabled={saving || status === gym.operationStatus} onClick={() => void saveStatus()} className="min-h-11 rounded-xl bg-slate-950 px-5 text-sm font-black text-white disabled:opacity-40">상태 저장</button><button disabled={saving} onClick={() => void verify()} className="min-h-11 rounded-xl border border-blue-200 bg-blue-50 px-5 text-sm font-black text-blue-700 disabled:opacity-40">정보 확인 완료</button></section>}
