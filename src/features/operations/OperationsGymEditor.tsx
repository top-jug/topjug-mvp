...
import { ArrowLeft, Bell, CheckCircle2, RefreshCw, Save } from 'lucide-react';
...
export function OperationsGymEditor() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><Link to="/ops/gyms" className="inline-flex items-center gap-1 text-sm font-bold text-slate-500 hover:text-blue-600"><ArrowLeft className="h-4 w-4" />암장 목록</Link><h2 className="mt-3 text-2xl font-black">{editing ? '암장 정보 편집' : '새 암장 등록'}</h2>{gym && <p className="mt-1 text-sm text-slate-500">ID {gym.id}</p>}</div>{gym && <div className="rounded-xl bg-slate-100 px-4 py-3 text-sm"><span className="font-bold text-slate-500">마지막 확인 </span><span className="font-black">{dateTime(gym.lastVerifiedAt)}</span><span className="mx-2 text-slate-300">·</span><span className="font-bold text-slate-500">수정 </span><span className="font-black">{dateTime(gym.updatedAt)}</span></div>}</div>
