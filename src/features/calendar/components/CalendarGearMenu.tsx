...
{gyms.map((gym) => (
              <button
                ...
                className={`py-3 rounded-full text-[15px] font-medium transition-colors flex items-center gap-2 ${
                  activeGyms[gym.name] ? 'bg-blue-500 text-white' : 'bg-neutral-100 text-neutral-700'
                }`}
              >
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold" style={{ backgroundColor: activeGyms[gym.name] ? 'rgba(255,255,255,0.3)' : gym.lightBg, color: activeGyms[gym.name] ? 'white' : gym.darkText }}>
                  {gym.name.slice(0, 1)}
                </div>
                <span>{gym.name}</span>
              </button>
