...
import { recordCountKey } from '../record-session-model';
  ...
  return (
    <div className="space-y-3">
      {difficulties.map((route) => {
        const key = recordCountKey(sectorId, route.id);
          ...
          <div key={key} className="bg-white rounded-xl p-3">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 min-w-[100px]">
                <div
                  className={`w-5 h-5 rounded-full border border-black/10 ${route.color.startsWith('bg-') ? route.color : ''}`}
                  style={route.color.startsWith('bg-') ? undefined : { backgroundColor: route.color }}
                />
