...
import { resumeRecordSession } from '../../../../../../../../src/server/records/record-session-service';
...
export const POST = withApiHandler<Context>(async (request, context) => {
  ...
  return NextResponse.json({ data: await resumeRecordSession(userId, parseInput(z.string().uuid(), recordId), input.at ? new Date(input.at) : undefined) });
});
