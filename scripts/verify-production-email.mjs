...
function requiredEmailAddress() {
  ...
  if (process.env.EMAIL_DELIVERY_MODE !== 'ses') {
    throw new Error('EMAIL_DELIVERY_MODE must be ses.');
  }
  return value;
}
...
async function main() {
  ...
  if (account.ProductionAccessEnabled !== true) throw new Error(`Amazon SES production access is not enabled in ${region}.`);
  if (identity.VerificationStatus !== 'SUCCESS') throw new Error(`Amazon SES identity ${identityDomain} is not verified.`);
  ...
  console.info(JSON.stringify({ event: 'production_email.preflight_succeeded', region, identityDomain }));
}
